...
const DEFAULT_CATEGORY: Partial<Category> = { CategoryName: "", Description: "", IsActive: true };
...
export default function CategoriesPage() {
  const {
    ...
    openViewDialog,
    openDeleteDialog,
  } = useMasterCRUD<Category>({
    endpoint: "categories",
    entityName: "Category",
    idField: "CategoryId",
    nameField: "CategoryName",
    defaultItem: DEFAULT_CATEGORY,
    ...
    <div className="flex flex-col h-full bg-card">
        ...
        <div className="border border-border rounded-xl overflow-hidden bg-background">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={7} className="h-32 text-center text-muted-foreground">Loading categories...</TableCell>
                </TableRow>
              ) : categories.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="h-32 text-center text-muted-foreground">No categories found.</TableCell>
                </TableRow>
              ) : (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{(currentItem as Category).CategoryId ? "Edit Category" : "Add New Category"}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-5 py-4">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-foreground">Category Name <span className="text-rose-500">*</span></label>
              <Input
                value={(currentItem as Category).CategoryName || ""}
            ...
            <div className="space-y-2">
              ...
              <textarea
                value={(currentItem as Category).Description || ""}
              ...
              <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-secondary/30">
                <div className="flex items-center gap-3">
                  <span className={cn(
                    ...
                    (currentItem as Category).IsActive ? "bg-emerald-500" : "bg-rose-500"
                  )} />
                  <span className={cn(
                    "text-sm font-semibold",
                    (currentItem as Category).IsActive ? "text-emerald-700 dark:text-emerald-400" : "text-rose-700 dark:text-rose-400"
                  )}>
                    {(currentItem as Category).IsActive ? "Active" : "Inactive"}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    {(currentItem as Category).IsActive ? "Category is visible and usable" : "Category is hidden from use"}
                  </span>
                </div>
                <button
                  type="button"
                  role="switch"
                  aria-checked={(currentItem as Category).IsActive}
                  onClick={() => setCurrentItem(prev => ({...prev, IsActive: !(prev as Category).IsActive}))}
                  className={cn(
                    ...
                    (currentItem as Category).IsActive ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"
                  )}
                >
                  <span
                    className={cn(
                      "pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow-lg ring-0 transition-transform duration-200 ease-in-out",
                      (currentItem as Category).IsActive ? "translate-x-5" : "translate-x-0"
                    )}
                  />
                </button>
              </div>
          ...
          <div className="space-y-6 py-4">
            <div className="space-y-1">
              ...
              <p className="font-mono text-lg font-semibold">
                CAT-{(currentItem as Category).CategoryId?.toString().padStart(5, '0')}
              </p>
            </div>
            <div className="space-y-1">
              <p className="text-sm font-medium text-muted-foreground">Category Name</p>
              <p className="text-lg font-medium">{(currentItem as Category).CategoryName}</p>
            </div>
            <div className="space-y-1">
              ...
              <div className="flex items-center">
                <span className={cn(
                  ...
                  (currentItem as Category).IsActive
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-[425px] border-rose-500/20">
          ...
          <div className="py-4 text-foreground/80">
            <p>Are you absolutely sure you want to delete the category <strong>{(itemToDelete as Category)?.CategoryName}</strong>?</p>
